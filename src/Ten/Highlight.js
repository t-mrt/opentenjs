...
Ten.Highlight = new Ten.Class({
    initialize: function (quote) {
        ...
        if (!c._cache)
            c._cache = {};
        if (c._cache[quote])
            return c._cache[quote];
        c._cache[quote] = this;
        c.makeTextNodes(c);
    },
    makeTextNodes: function (c) {
        if (c.textNodes || c.textNodePositions || c.documentText)
            return;
    ...
    makeMatchedNodes: function () {
        ...
        var i = 0;
        for (var start = text.indexOf(quote); start != -1; start = text.indexOf(quote, start + quote.length)) {
    ...
    replaceNode: function (i, show) {
        ...
        if (newNode)
            Ten.DOM.replaceNode(newNode, oldNode);
        c._lock[i] = false;
    },
    containers: null,
    matchedNodes: null
