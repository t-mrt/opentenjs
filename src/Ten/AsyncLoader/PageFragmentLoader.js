Ten.AsyncLoader.PageFragmentLoader = new Ten.Class({
    ...
    _initWithURL: function (url) {
        ...
        if (Hatena.Locale) {
            if (!/local.hatena/.test(location.hostname)) {
    ...
    setLoadStartEnd: function (loadStart, loadEnd) {
        if (loadStart)
            this._loadStart.push(loadStart);
        if (loadEnd)
            this._loadEnd.push(loadEnd);
        return this;
    },
    start: function (code) {
        var self = this;
        if (!this.noIndicator)
            Ten.AsyncLoader.Indicator.start(this.indicatorKey);
    ...
    _onload: function (xhr, code) {
        ...
        if (code)
            code.apply(this, []);
        if (!this.noIndicator)
            Ten.AsyncLoader.Indicator.stop(this.indicatorKey);
