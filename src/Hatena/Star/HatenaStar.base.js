...
Hatena.Star.Entry = new Ten.Class({
    ...
    addAddButton: function () {
        var addButtonClass = this.constructor.AddButtonClass || ((Hatena.Star.useSmartPhoneStar && Hatena.Star.isTouchUA)
    ...
    removeTemporaryStar: function () {
        if (this.temporaryStars) {
            ...
            if (star)
                this.star_container.removeChild(star);
            return star;
        }
        return null;
    },
...
Hatena.Star.AddButton = new Ten.Class({
    ...
    clearSelectedColorTimer: function () {
        try {
            clearTimeout(this.showSelectedColorTimerId);
        }
        catch (e) { }
        ;
        try {
            clearTimeout(this.hideSelectedColorTimerId);
        }
        catch (e) { }
        ;
    },
...
Hatena.Star.Pallet = new Ten.Class({
    ...
    showSelectedColor: function (pos, addButton) {
        ...
        if (pos)
            this.selected_color_pos = pos;
        var iframeStyle;
        if (Ten.Browser.isIE)
            iframeStyle = "width:16px;height:5px;border:1px solid #bbbbbb;";
        else
            iframeStyle = "width:14px;height:3px;border:1px solid #bbbbbb;";
...
Hatena.Star.Pallet.SmartPhone = new Ten.Class({
    ...
    showPallet: function (pos, addButton) {
        ...
        if (Ten.Browser.isDSi) {
            ...
            new Ten.Observer(star_yellow, 'onmouseup', function (e) {
                img._activeTimer = setTimeout(function () { img.parentNode.className = ''; }, 100);
            });
        }
        new Ten.Observer(this.pallet, 'onload', this, 'observerSelectColor');
        var self = this;
        window.addEventListener("message", function (e) { if (e.data == 'sending')
            self.sending(); }, false);
                ...
                try {
                    ...
                    for (var y = ys; y < ye; y += 5) {
                        for (var x = xs; x < xe; x += 5) {
                            ...
                            else if ((a = ancestor(e, 'A', 3))) {
                                if (a._checked)
                                    continue;
                                a._checked = true;
                                self._checkedElements.push(a);
    ...
    hide: function (e) {
        ...
        if (Ten.Browser.isAndroid) {
            try {
                ...
                if (this._checkedElements)
                    for (var i = 0, len = this._checkedElements.length; i < len; i++) {
    ...
    showCloseButton: function () {
        if (!this.closeButton) {
            var closeButton = Ten.Element('a', { href: 'javascript:void(0)' }, Ten.Element('img', { src: this.constructor.getColorSchemeItem('getCloseButtonImage')(), style: { verticalAlign: 'middle', padding: this.constructor.getColorSchemeItem('closeButtonImagePadding') } }), Hatena.Star.Text.close || 'Close');
...
Hatena.Star.CommentButton = new Ten.Class({
    ...
    showComments: function (e) {
        if (!this.screen)
            this.screen = new Hatena.Star.CommentScreen();
...
Hatena.Star.Star = new Ten.Class({
    ...
    showName: function (e) {
        if (!this.screen)
            this.screen = new Hatena.Star.NameScreen();
        var pos = e.mousePosition();
        pos.x += 10;
        pos.y += 25;
        if (this.highlight)
...
Hatena.Star.Highlight.makeTextNodes = function (c) {
    if (c.asyncMakeTextNode || c.textNodes || c.textNodePositions || c.documentText)
        return;
    if (Ten.Highlight.highlighted)
        Ten.Highlight.highlighted.hide();
    ...
    c.asyncMakeTextNode = function () {
        ...
        if (Ten.Browser.isSupportsXPath) {
            ...
            var fn = function (node, parent) {
                if (isIE && parent && parent != node.parentNode)
                    return;
                if (node.nodeType == 3) {
                    textNodes.push(node);
...
Hatena.Star.CommentScreen = new Ten.Class({
    ...
    initialize: function () {
        ...
        if (!self.commentsContainer)
            self.addCommentsContainer();
        return self;
    },
    style: {
        width: '280px',
...
Hatena.Star.EntryLoader = new Ten.Class({
    ...
    receiveStarEntries: function (res) {
        ...
        for (var i = 0, len = entries.length; i < len; ++i) {
            ...
            if (!eURI)
                eURI = entryInfo.eURI = encodeURIComponent(entryInfo.uri);
            encodedUriToEntryInfoMap[eURI] = entryInfo;
        }
        for (var i = 0, len = c.entries.length; i < len; ++i) {
            var e = c.entries[i];
            var entryInfo;
            if (e.hasBoundToStarEntry())
                continue;
            if (!e.eURI)
                e.eURI = encodeURIComponent(e.uri);
