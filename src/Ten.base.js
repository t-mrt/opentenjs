...
if (typeof (Ten) == 'undefined') {
    ...
    Ten.Class.inherit = function (child, parent) {
        for (var prop in parent) {
            if (typeof (child[prop]) != 'undefined' || prop == 'initialize')
                continue;
    ...
    */
    ...
    Ten.JSONP = new Ten.Class({
        initialize: function (uri, obj, method) {
            ...
            if (obj && method)
                Ten.JSONP.addCallback(obj, method);
    ...
    Ten.DOM = new Ten.Class({
                ...
                if (str.indexOf("<") == 0) {
                    if ((t = str.match(/^\s*<(\/?[^\s>\/]+)([^>]+?)?(\/)?>/))) {
                        ...
                        if (tag.indexOf("/") == -1) {
                            ...
                            if (attrs)
                                attrs.replace(/([a-z]+)=(?:'([^']+)'|"([^"]+)")/gi, function (m, name, v1, v2) {
                                    var v = text(v1 || v2);
                                    if (name == "class")
                                        root && (root[v] = child), child.className = v;
                    ...
                    if ((t = str.match(/^([^<]+)/)))
                        cur.appendChild(document.createTextNode(text(t[0])));
                }
        ...
        addClassName: function (element, className) {
            if (Ten.DOM.hasClassName(element, className))
                return;
    ...
    Ten.SelectorNode = new Ten.Class({
        ...
        parseSelector: function () {
            ...
            if (this.option && this.option.separator)
                this.parseSeparator();
            if (this.pseudoClass)
                this.parsePseudoClass();
        },
        ...
        getElements: function (parent) {
            if (typeof this.getElementsBySelector != 'function')
                return null;
            var ret = [];
    ...
    Ten._Selector = new Ten.Class({
        ...
        getElementsBySelector: function (selector, parent) {
            ...
            if (selector.indexOf(',') >= 0)
                ret.sort(Ten._Selector._sortByElementOrder);
            return ret;
        },
        _sortByElementOrder: function (a, b) {
            ...
            if (depthA > depthB)
                for (var i = 0; i < (depthA - depthB); i++)
                    a = a.parentNode;
            else if (depthA < depthB)
                for (var i = 0; i < (depthB - depthA); i++)
                    b = b.parentNode;
                ...
                this[f] = function (parent) {
                    ...
                    for (var j = 0, len = parents.length; j < len; j++) {
                        ...
                        for (var n = 0; n < children.length; n++) {
                            ...
                            if (i < 0)
                                continue;
                            if (children[i] && children[i].tagName == tagName)
                                ret.push(children[i]);
                        }
                    }
                    return ret;
    ...
    if (document.querySelectorAll) {
        Ten.querySelectorAll = function (selector, elem) {
            ...
            try {
                if (elem)
                    elems = (elem.querySelectorAll) ? elem.querySelectorAll(selector) : [];
                else
